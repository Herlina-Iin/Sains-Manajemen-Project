// EA07_ADX_TrendFilter.mq5
// MA crossover filtered by ADX trend strength.
...
input int FastMA=20; input int SlowMA=50; input int ADXPeriod=14; input double MinADX=20;
int hf,hs,ha;
int OnInit(){ hf=iMA(_Symbol,_Period,FastMA,0,MODE_EMA,PRICE_CLOSE); hs=iMA(_Symbol,_Period,SlowMA,0,MODE_EMA,PRICE_CLOSE); ha=iADX(_Symbol,_Period,ADXPeriod); return (hf<0||hs<0||ha<0)?INIT_FAILED:INIT_SUCCEEDED; }
void OnTick(){
 if(!IsNewBar() || (InpOnePosition&&HasPosition()))return;
 double f[3],s[3],a[3]; if(CopyBuffer(hf,0,0,3,f)<3||CopyBuffer(hs,0,0,3,s)<3||CopyBuffer(ha,0,0,3,a)<3)return;
 if(a[1]<MinADX)return;
